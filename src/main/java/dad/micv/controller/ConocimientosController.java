...
import dad.micv.App;
import dad.micv.dialog.ConocimientoDialog;
import dad.micv.dialog.IdiomaDialog;
...
public class ConocimientosController implements Initializable {
	...
	private ListProperty<Conocimiento> conocimientosList = new SimpleListProperty<>(FXCollections.observableArrayList());
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		eliminarButton.disableProperty().bind(conocimientosTableView.getSelectionModel().selectedItemProperty().isNull());
		
		// cell value factories
		
		denominacionColumn.setCellValueFactory(v -> v.getValue().denominacionProperty());
//		denominacionColumn.setCellValueFactory(v -> v.getValue().idiomaProperty().getValue().certificacionProperty());
    ...
    void onEliminarAction(ActionEvent event) {
		...
		alert.setContentText(
				"¿Quiere eliminar el siguiente conocimiento? " + conocimientosTableView.getSelectionModel().getSelectedItem().getDenominacion());
